...
1.  Western Cape Provincial Land Cover (Cape Nature, 2022).

2.  KwaZulu-Natal Provincial Land Cover (Ezemvelo KZN Wildlife, 2017).

3.  Mpumalanga Provincial Land Cover (MPTA, 2017).
...
alt_sum <- alt_rall %>%
  ...
  pivot_wider(names_from = LCcomb, values_from = km2tot) %>%
    mutate(prplost = notnat/(nat+notnat)) %>% # ext of remnants (A3)
