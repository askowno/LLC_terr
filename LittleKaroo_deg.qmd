...
LK <- rast("C:/Users/skownoa/Dropbox/NBAwork/RLEworking/Alternative_LC_DEG/littlekaroo_deg4.tif")
...
write.csv(lk_rall, file = "outputs/lk_rall2.csv")
...
lk_sum <- lk_rall %>%
  ...
  pivot_wider(names_from = LCcomb, values_from = km2tot) %>%
    mutate(prpdeg = deg80/(nat+deg80)) %>% # cond of remnants (D1&3)
    mutate(D3_lk = case_when(prpdeg >= 0.9 ~ "CR",
                   prpdeg >= 0.7 ~ "EN",
                   prpdeg >= 0.5 ~ "VU",
                  TRUE ~ "LC")) %>%
